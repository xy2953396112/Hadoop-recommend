...
public class Step3 {
     ...
     private final static IntWritable V = new IntWritable(1);   //Step3阶段的计数器
	 ...
	 static class Step3_Mapper extends Mapper<LongWritable, Text, Text, IntWritable>{
		...
		protected void map(LongWritable key,Text value,Context context) throws IOException, InterruptedException {
			...
			for (int i = 0; i < items.length; i++) {
				String itemA = items[i].split(":")[0];   //itemA为每个物品的id
				for (int j = 0; j < items.length;j++) {
					String itemB = items[j].split(":")[0];
					K.set(itemA+":"+itemB);    
					//K为不同物品   itemA:itemB   V为计数器1
					context.write(K,V);  
				}
			}
